...
-(void) reload {
    ...
    ds.statusText = [NSString stringWithFormat:NSLocalizedString(@"reloading",@"transmitter: reloading")];
    ds.device = nil;
...
-(void)btStatus:(NSNotification*)notification {
    if([((BluetoothStatus*)[notification object]) state] == CBManagerStatePoweredOn) {
        ...
        NSUUID* lastUsed = [[NSUUID alloc] initWithUUIDString:[deviceData objectForKey:@"transmitterUUID"]];
        if(lastUsed) {
            [[NSNotificationCenter defaultCenter] postNotificationName:BLUETOOTH_RESTORE_DEVICE object:lastUsed];
        } else {
            DeviceStatus* ds = [[DeviceStatus alloc] init];
            ds.status = NO_DEVICE;
            ds.statusText = [NSString stringWithFormat:NSLocalizedString(@"Not able to connect to %@",@"transmitter: Not able to connect to device"),[lastUsed UUIDString]];
            ds.device = nil;
            [[NSNotificationCenter defaultCenter] postNotificationName:kDeviceStatusNotification object:ds];
            self.lastDeviceStatus = ds;
        }
...
-(void) deviceRestored:(NSNotification*) notification {
    if([notification object]) {
        ...
        ds.statusText = [NSString stringWithFormat:NSLocalizedString(@"Connecting to %@",@"transmitter: connecting to device"),[[notification object] name]];
        ds.device = nil;
        [[NSNotificationCenter defaultCenter] postNotificationName:kDeviceStatusNotification object:ds];
        self.lastDeviceStatus = ds;
        ...
        NSDictionary* deviceData = [[Storage instance] deviceData];
        NSUUID* lastUsed = [deviceData objectForKey:@"transmitterUUID"];
        if(lastUsed) {
            [[NSNotificationCenter defaultCenter] postNotificationName:BLUETOOTH_RESTORE_DEVICE object:lastUsed];
        }
    }
...
-(void) disconnected:(NSNotification*) notification {
    if(![self device] || [[self device].identifier isEqual:((CBPeripheral*)[notification object]).identifier]) {
        DeviceStatus* ds = [[DeviceStatus alloc] init];
        ds.status = DEVICE_DISCONNECTED;
        ds.statusText = [NSString stringWithFormat:NSLocalizedString(@"Disconnected from %@, waiting for next",@"transmitter: disconnect from device"),[[notification object] name]];
        ds.device = nil;
        [[NSNotificationCenter defaultCenter] postNotificationName:kDeviceStatusNotification object:ds];
        self.lastDeviceStatus = ds;
...
-(void) failed:(NSNotification*) notification {
    if(![self device] || [[[self device] identifier] isEqual:[((CBPeripheral*)[[notification object] objectForKey:@"peripheral"])identifier]]) {
        DeviceStatus* ds = [[DeviceStatus alloc] init];
        ds.status = DEVICE_DISCONNECTED;
        ds.statusText = [NSString stringWithFormat:NSLocalizedString(@"failed connection to %@",@"transmitter: failed connecting to device"),[[[notification object] objectForKey:@"peripheral"] name]];
        ds.device = nil;
        [[NSNotificationCenter defaultCenter] postNotificationName:kDeviceStatusNotification object:ds];
        self.lastDeviceStatus = ds;
...
-(void) deviceDiscovered:(NSNotification*) notification {
    if([[[notification object] objectForKey:@"peripheral"] isEqual:self.device]) {
        ...
        if([service.UUID isEqual:[CBUUID UUIDWithString:BLUETOOTH_SERVICE_RFDUINO_CUSTOM_SERVICE]]) {
            for(CBCharacteristic* chara in service.characteristics) {
                if([chara.UUID isEqual:[CBUUID UUIDWithString:BLUETOOTH_SERVICE_RFDUINO_CUSTOM_SERVICE_RX]]) {
                    [self.device setNotifyValue:YES forCharacteristic:chara];
                    _rx = chara;
                }else if([chara.UUID isEqual:[CBUUID UUIDWithString:BLUETOOTH_SERVICE_RFDUINO_CUSTOM_SERVICE_TX]]) {
                    [self.device setNotifyValue:YES forCharacteristic:chara];
                    _tx = chara;
                }else {
            ...
            if(_rx) {
                ...
                ds.statusText = [NSString stringWithFormat:NSLocalizedString(@"Connected to %@",@"transmitter: connected device"),[self.device name]];
...
-(void)dataRecieved:(NSNotification*)notification {
    if([[self device] isEqual:[notification.object objectForKey:@"peripheral"]]) {
        if([[[notification object] objectForKey:@"characteristic"] isEqual:_rx]) {
            ...
            if([transmitter isAllDigits:trimmed] && [trimmed length]>3)
            {
                int limitterValue = 0;
                int battery = 0;
                int minutes = 0;
                ...
                if(found) {
                    NSString* rawSource = @"transmitter";
                    [[Storage instance] addBatteryValue:battery raw:battery source:@"transmitter" device:[self class]];

                    batteryValue* bV = [[batteryValue alloc] initWith:battery raw:battery from:@"transmitter" class:[self class] at:[[NSDate date] timeIntervalSince1970]];
                    ...
                    DeviceStatus* ds = [[DeviceStatus alloc] init];
                    ds.status = DEVICE_OK;
                    ds.statusText = NSLocalizedString(@"transmitter ok",@"transmitter: sensor ok");
                    ds.device = self.device;
                    [[NSNotificationCenter defaultCenter] postNotificationName:kDeviceStatusNotification object:ds];
                    self.lastDeviceStatus = ds;
                }
                    ...
                    ds.statusText = NSLocalizedString(@"transmitter failed",@"transmitter: no data found");
                    ds.device = self.device;
                    [[NSNotificationCenter defaultCenter] postNotificationName:kDeviceStatusNotification object:ds];
                    self.lastDeviceStatus = ds;
            }
...
+(NSString*) configurationName {
    return @"Mareks Transmitter";
}

+(NSString*) configurationDescription {
    return NSLocalizedString(@"The Transmitter is used to fetch values from the libre Tag.",@"transmitter: Description");
}
...
+(void) selectedDevice:(NSNotification*)notification {
    ...
    if(peripheral) {
        [deviceData setObject:[[peripheral identifier] UUIDString] forKey:@"transmitterUUID"];
    } else {
        [deviceData removeObjectForKey:@"transmitterUUID"];
    }
...
-(NSArray*) getRequestedDeviceUUIDs {
    return [NSArray arrayWithObjects:[CBUUID UUIDWithString:BLUETOOTH_SERVICE_RFDUINO_CUSTOM_SERVICE], nil];
}
